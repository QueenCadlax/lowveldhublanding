...
export const EarlyAccess: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setStatus('success');
        setMessage(data.message || 'Your seat among the elite is reserved.');
        setEmail('');
      } else {
        setStatus('error');
      ...
      setStatus('error');
      setMessage('Our systems are currently exclusive. Please try again.');
    }
  };
  ...
  return (
    <section id="early-access" className="py-40 bg-luxury-dark relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
        <h2 className="text-4xl md:text-7xl font-serif font-light mb-6">Get <span className="italic text-gold-gradient">Early Access</span></h2>
        <p className="text-lg md:text-xl font-sans font-extralight text-white/60 mb-16 leading-relaxed max-w-2xl mx-auto">
          Be the first to experience Mpumalanga’s premium digital ecosystem.
        </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="flex flex-col sm:flex-row gap-4 p-2 bg-white/[0.02] border border-white/10 backdrop-blur-xl shadow-2xl transition-all duration-500 hover:border-luxury-gold/30">
              ...
              <button 
                type="submit"
                disabled={status === 'loading' || status === 'success'}
                className={`px-10 py-5 bg-luxury-gold text-black font-sans font-bold text-xs uppercase tracking-[0.4em] transition-all duration-700 whitespace-nowrap ${
                  status === 'success' 
            ...
            {message && (
              <p className={`text-[10px] uppercase tracking-[0.3em] font-sans italic animate-in fade-in slide-in-from-top-2 duration-500 ${status === 'error' ? 'text-red-400' : 'text-luxury-gold'}`}>
                {message}
              </p>
            )}
          </form>
